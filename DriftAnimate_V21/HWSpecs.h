...
typedef hardwarespecs {
  ...
  uint8_t LCD_flags;      //
  uint8_t RF_Flags;       //
  uint8_t Basic_Features; //  (tentative - circit protection and connections present
  uint8_t UI_connections; //  (tebtative - invert encoders and such)
  uint8_t data_config;    //
  uint8_t RF_config;      //
...
extern hardware_t Hardware;
  ...
  PROGMEM_MAPPED char  names_receivers[8][9] = {"None", "SYN470", "SRX882", "SuperHet", "SuperReg", "H5V4D", "STX887", "Resvd."};
